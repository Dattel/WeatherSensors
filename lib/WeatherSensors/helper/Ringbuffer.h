...
public:
    Ringbuffer();
    bool push(const BUFFTYPE * value) __attribute__ ((noinline));
    bool pull(BUFFTYPE &value) __attribute__ ((noinline));
    bool contains(const BUFFTYPE * value) __attribute__ ((noinline));
    void clear()   { size = 0; }
...
template <typename BUFFTYPE, uint16_t BUFFSIZE>
bool Ringbuffer<BUFFTYPE, BUFFSIZE>::push(const BUFFTYPE * value)
{
    if (size == BUFFSIZE) 
        return false;
...
template <typename BUFFTYPE, uint16_t BUFFSIZE>
bool Ringbuffer<BUFFTYPE, BUFFSIZE>::contains(const BUFFTYPE * value)
{
    if (size == 0) 
        return false;
    for (uint16_t i = 0, j = readPos; i < size; i++) {
      if (j == BUFFSIZE) 
        j = 0;
      Serial.printf("Check RawValue %llu against %llu\n", buffer[j++].rawData, value->rawData);
      if (buffer[j++].rawData == value->rawData)
        return true;
    }
